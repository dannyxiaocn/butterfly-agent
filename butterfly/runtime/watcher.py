...
class SessionWatcher:
    ...
    async def _scan(self) -> list[str]:
        ...
        for system_dir in sorted(self.system_sessions_dir.iterdir()):
            ...
            from butterfly.session_engine.session_status import read_session_status, write_session_status, pid_alive
            status_data = read_session_status(system_dir)

            # Skip sessions whose daemon is already running (e.g. started by
            # `butterfly chat`). Without this check, the watcher would start a
                ...
                if stopped_at_str:
                    try:
                        ...
                        if elapsed >= _AUTO_EXPIRE_HOURS * 3600:
                            ...
                            from butterfly.session_engine.task_cards import clear_all_cards
