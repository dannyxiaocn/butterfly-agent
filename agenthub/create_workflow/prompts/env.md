...
**bash default directory**: `sessions/{session_id}/` — when you save a
new workflow you must write outside this dir, into `<repo_root>/workflows/`.
Resolve `<repo_root>` once with `bash: git rev-parse --show-toplevel` and
use absolute paths from there for `write`.
