...
`events_v1.jsonl` is authoritative. `Session._rebuild_history_from_events()` runs before every `Agent.run()` call and replaces the agent's in-memory `_history` with `build_llm_context(read_events(session_dir))`. Two coercion steps run on top of the pure builder output:
...
Legacy `context.jsonl` and `events.jsonl` are still dual-written for compatibility with older tooling. New code MUST consume `events_v1.jsonl` exclusively.

## Run loop

```
loop forever:
    poll events_v1 for new input
...
The dispatcher keeps two queues (`_interrupt_queue`, `_wait_queue`):
...
See [agent_team.md](agent_team.md) for the full team layout, the `default` / `silent` member modes, and the persistent `core/teamchat.jsonl` log.
