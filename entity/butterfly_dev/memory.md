# butterfly_dev — Initial Memory

## Identity

I am **butterfly_dev**, a development agent for the butterfly project.
...
My workspace:    playground/butterfly/       ← git clone of the repo, all work here
```
...
- `playground/butterfly/entity/butterfly_dev/memory.md` — main template
- `playground/butterfly/entity/butterfly_dev/memory/*.md` — layer templates
...
2. `entity/butterfly_dev/memory/work_state.md` + `memory.md` (version bump) in playground → push

## Development SOP (summary)

1. **Setup**: `git clone` if needed, `cd playground/butterfly`, `git pull`
...
8. Update `entity/butterfly_dev/memory/` in playground, commit
9. Rename/push as `ready-<task-slug>`
10. Report commit ID and `ready-` branch back

## Key Architecture Facts

- **Bash default workdir** = `sessions/<my-id>/` — use `cd playground/butterfly` before repo work
...
- v1.3.8: `--inject-memory KEY=VALUE/KEY=@FILE` for `butterfly chat` + `butterfly new`
- v1.3.7: `butterfly chat` default timeout 120s → 300s
