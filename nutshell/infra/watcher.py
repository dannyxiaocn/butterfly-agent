...
class InstanceWatcher:
    ...
    async def _scan(self) -> None:
        """Scan instances_dir for new or recovered manifests."""
        ...
        for instance_dir in sorted(self.instances_dir.iterdir()):
            ...
            print(f"[server] Discovered instance: {instance_id}")
    ...
    async def _start_instance(
        ...
        existing = instance_dir / "context.json"
        if existing.exists() and existing.stat().st_size > 2:
            print(f"[server] Resumed instance: {instance_id}")
        else:
            print(f"[server] Created instance: {instance_id}")
