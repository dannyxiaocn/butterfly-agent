...
def test_token_report_default_session_skips_meta(tmp_path, capsys):
    sessions_base, system_base = _make_session(tmp_path, "agent_meta")
    _write_context(system_base, "agent_meta", [
        {"type": "turn", "messages": [], "ts": "2026-01-01T00:00:00", "usage": {"input": 1, "output": 1}},
    ])

    _make_session(tmp_path, "chat-session")
    _write_context(system_base, "chat-session", [
        {"type": "turn", "messages": [], "ts": "2026-01-01T00:00:00", "usage": {"input": 2, "output": 2}},
    ])

    args = _Args(None, sessions_base, system_base)
    ...
    assert rc == 0
    out = capsys.readouterr().out
    assert "[chat-session] token-report" in out
    assert "[agent_meta] token-report" not in out
