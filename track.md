...
- [ ] git 工作区
    - [ ] [WIP] 在工作，没有的时候都是想要提交？要让 agent 来进行轮询？
...
- [ ] 环境与反馈系统
- [ ] 约束系统 — sandbox 等
- [ ] 计算 token 数，让模型根据这个找到更短的路径 *(token 追踪已完成 <!-- 8c4b494 v1.2.7 -->；基于 token 的路径优化待做)*
...
- [ ] 先实现一个实时通信的，用 skill 来实现
    - [ ] 上线一个动作，获取当前的所有好友（以及在线和离线状态），发送实时信息
...
- [ ] Tool 作为 filesystem 的主要交互，内容肯定是不能直接丢弃的，但是对于很多程序化生成的，比如看系统状态等等的工具，能否更 fine-grained 只描述差异（维护一个全表格，只增加一个差异来表示呢？）—> 对于高频 tool 更 fine-grained：1. 为了效果；2. 为了成本
- [ ] Prompt 优化 —> 效果 vs 成本 —> 如何更好的规划这个 prompt 的空间，以及计算这样的意义
